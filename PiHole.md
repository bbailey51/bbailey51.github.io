...
![Image of a Raspberry Pi 4 Model B](/20260822_104828.jpg)
...
![Pi Imaging Start](/PiHoleImaging.png)
![Pi Imaging Finished](/PiHoleImagingfinished.png)
...
![PuTTY Setup](/PuTTYSetup.png)
![SSH logged in](/PuTTYloggedin.png)
![PiHoleInstallation](/InstallPiHole.png)
![PiHoleComplete](/InstallPiHoleFinished.png)
...
![PiHoleComplete](/piholelisting.png)
![PiHoleComplete](/gravitylists.png)
...
![PiHoleComplete](/dnssettings.png)
<!-- Add in the future before and after of sites with ads -->
...
7.	Due to using Xfinity as the ISP and their router/modem, prevents IP reservations (important to keep the Pi-Hole in 1 address so not chasing the address around on individual devices), will not give access to change the DNS server it’s pointed to (set to Xfinity’s default servers address of 75.75.75.75 & 76.76.76.76, instead of to the Pi-Hole).  
    ...
    - Could potentially do a workaround setting the Pi-Hole as the DHCP server however the Xfinity router will still have a chance to serve as the DHCP server since I cold only limit what IP addresses it could serve and best to limit it is 1 address and could refresh it every 5 minutes to make sure it doesn’t have a hold of it long if it is able to serve an IP address to a device before the Pi-Hole can

8.	Due to this limitation, full implementation hasn’t been put in place until maintenance with Xfinity is completed due to some interference & speed issues yet to be figured out though everything has worked up to implementation on the router side of things and know what steps to take next with IP reservation of the Pi-Hole and changing the DNS server to that address
   
9.	The ideal Plan A is get a stronger router that allows for customization to hook into the Xfinity gateway and set it to bridge mode to become a modem only and then implement the Pi-Hole without any of the Xfinity restrictions along with using my own choice of a router, also thinking for myself and the person who has ownership of the gateway that I could potentially get a better network and coverage with the plan of the router and Wi-Fi extender working together as 1 network.
